...
export const updateCoordinateEpic = action$ =>
    action$.pipe(
        ofType(ActionEvent.UPDATE_COORDINATE),
        switchMap((action) =>
            fetch(ServerPath.API_URL + 'location/update/coordinate', {
                method: 'POST',
                headers: ApiUtil.getHeader(),
                body: JSON.stringify(action.payload)
            }).then((response) => {
                ...
                console.log(responseJson);
                return location.updateCoordinateSuccess(responseJson);
            }).catch((error) => {
                consoleLogEpic("UPDATE_COORDINATE USER_EPIC:", ActionEvent.UPDATE_COORDINATE, error);
                return handleConnectErrors(error)
            })
        )
    );