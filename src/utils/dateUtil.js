...
export default class DateUtil {
    ...
    static timeAgo(time) {
        ...
        let formatDay = DateUtil.convertFromFormatToFormat(time, DateUtil.FORMAT_DATE_TIME_ZONE, DateUtil.FORMAT_DATE);
        let formatHour = DateUtil.convertFromFormatToFormat(time, DateUtil.FORMAT_DATE_TIME_ZONE, DateUtil.FORMAT_TIME);
        var date = new Date(formatTime);
        var diff = (((currentDateTime.getTime() - date.getTime()) / 1000));
        var day_diff = Math.floor(diff / 86400);
        if (isNaN(day_diff) || day_diff < 0)
            return localizes('timeAgo.just')
        return day_diff == 0 && (
            // diff < 60 && localizes('timeAgo.just') ||
            // diff < 120 && localizes('timeAgo.oneMinuteAgo') ||
            // diff < 3600 && Math.floor(diff / 60) + localizes('timeAgo.minAgo') ||
            // diff < 7200 && localizes('timeAgo.oneHoursAgo') ||
            diff < 86400 && localizes('timeAgo.toDay')) ||
            day_diff == 1 && localizes('timeAgo.yesterday') ||
            day_diff < 7 && formatDay ||
            day_diff < 31 && formatDay ||
            day_diff > 31 && formatDay
    }
